...
- `self.mode` is one of `"view" | "schema" | "sql" | "diagram"`, cycled with `m` (skipping `"diagram"` if `provider.capabilities.diagram` is `False`); `ctrl+d`/`command+d` jumps straight to diagram mode (or notifies it's unavailable). `load_item()` is the central dispatcher that reads `self.mode` + the selected sidebar item's type (`"table" | "view" | "plugin"`) and populates the right widget via `self.provider.*` calls. Bare `d` is `delete_item` instead (issue #10 follow-up) — `m`-cycling plus per-object direct-jump/edit keys is expected to eventually replace `self.mode` entirely (see the open design note for a `table/edit`, `view/edit`, `column/edit`, `database/edit` object+action vocabulary), but that's not implemented yet.
...
- Which of those bindings actually appear in Textual's `Footer` (and are live/dead) is context-aware, driven by `DbMan.check_action()` overriding the base no-op — Textual's built-in dynamic-actions hook, read from `Screen.active_bindings` any time `refresh_bindings()` is called. `DbMan.ACTION_CONTEXTS` maps an action name (not a key — `alt+left`/`alt+right` both resolve to `"reorder_column"`) to a predicate `(app) -> bool | None`: `True` shows it enabled, `False` hides it from the footer entirely, `None` shows it greyed out (visible but currently inert, e.g. `u`/unhide when there are no hidden columns to restore). Most predicates are built with the `_ctx(modes=..., select_modes=..., item_types=..., capability=...)` factory (an AND of whichever axes are given); `_edit_cell_ctx` is hand-written instead since `e` is genuinely polymorphic (SQL mode edits the View's SQL, the lookup plugin edits its own config, View mode's behavior further forks on `self.select_mode`) and doesn't reduce to a plain AND. Actions with no entry in `ACTION_CONTEXTS` are always shown+enabled. Any code path that changes `self.mode`, `self.select_mode`, or `self.current_item`/`self.current_type` must call `self.refresh_bindings()` afterward (already wired into `load_item()`, `action_toggle_mode`, `action_rotate_select_mode`, `action_change_mode_diagram`) or the footer will visibly lag the real state. See issue #10.
...
- In-app UI now covers all three: `w` sets/clears a width override (`action_set_column_width`), `option+left`/`option+right` in column select mode reorders (`action_reorder_column`, see below), and `z`/`u` hide/unhide a column (`action_hide_column`/`action_unhide_column`, issue #2). `z`, `w`, `f` (filter), and `u` (via `UnhideColumnScreen`, a `Select`-based picker) are all column-select-mode-scoped — column-granularity actions only make sense once a column is the thing selected, consistent with issue #10's context system (`u` additionally greys out via `_unhide_column_ctx` when there's nothing hidden to restore). `action_hide_column` refuses to hide a table/view's last remaining visible column.
