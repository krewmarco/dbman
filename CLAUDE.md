...
- In-app UI now covers all three: `w` sets/clears a width override (`action_set_column_width`), `option+left`/`option+right` in column select mode reorders (`action_reorder_column`, see below), and `z`/`u` hide/unhide a column (`action_hide_column`/`action_unhide_column`, issue #2). `z` is column-select-mode-scoped like reordering; `u` (via `UnhideColumnScreen`, a `Select`-based picker) is mode-independent since a hidden column isn't reachable through column-mode cursor selection by definition. `action_hide_column` refuses to hide a table/view's last remaining visible column.
